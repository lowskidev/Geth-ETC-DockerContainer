...
function dockerRUN(){
	...
	if [[ $instType == 'd' ]]; then
		if [[ $exposeRPC == 'y' ]]; then
			sudo docker run -ti --name $containerName -p $gethPort:30303/tcp -p $gethPort:30303/udp -p $rpcPort:8545/tcp --restart unless-stopped --mount type=bind,source=$HOME/.ethereum-classic/$containerName,target=/.ethereum-classic/ ${imageName,,}
		else
			sudo docker run -ti --name $containerName -p $gethPort:30303/tcp -p $gethPort:30303/udp --restart unless-stopped --mount type=bind,source=$HOME/.ethereum-classic/$containerName,target=/.ethereum-classic/ ${imageName,,}
		fi
	else
		if [[ $exposeRPC == 'y' ]]; then
			sudo docker run -ti --name $containerName -p $gethPort:30303/tcp -p $gethPort:30303/udp -p $rpcPort:8545/tcp --restart unless-stopped --mount type=bind,source=$HOME/.ethereum-classic/$containerName,target=/.ethereum-classic/ bakon3/etcnode
		else
			sudo docker run -ti --name $containerName -p $gethPort:30303/tcp -p $gethPort:30303/udp --restart unless-stopped --mount type=bind,source=$HOME/.ethereum-classic/$containerName,target=/.ethereum-classic/ bakon3/etcnode
		fi
	fi	
}
